...
[McpServerPromptType]
internal sealed class InvestigatePackagePrompt
{
    [McpServerPrompt(Name = "investigate_package")]
    ...
    public static string Investigate(
        [Description("Package name or search query, e.g. 'Newtonsoft.Json' or 'json serializer'")] string package,
        [Description("What to find out, e.g. 'how does JsonConvert.SerializeObject work?' or 'what configuration options exist?'")] string? goal = null)
    {
        var goalClause = goal is not null
            ? $"\n\nThe user's specific goal: {goal}"
            : "";

        return $"""
            Investigate the NuGet package: {package}{goalClause}

            Follow this workflow, stopping as soon as the goal is satisfied:

            1. If "{package}" looks like a search query rather than an exact package ID, call `nuget_search` to find the right package ID.
